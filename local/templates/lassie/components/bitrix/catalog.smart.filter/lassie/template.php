...
<form action="<?= $arResult["FORM_ACTION"] ?>" style="width:270px" class="catalog-page__filter catalog__filter form"
    ...
    <fieldset class="form__fieldset">
        <legend style="float: none;"
                class="form__title form__title_align_center"><?= ("Фильтр"); //GetMessage("CT_BCSF_FILTER_TITLE")
          ?></legend>
      <?
      ...
      foreach ($arResult["ITEMS"] as $key => $arItem) {
        ...
        <?
      if ($arItem["CODE"] == "INPUT_SIZE_NUMERIC" || $arItem["CODE"] == "color_new") {
        ?>
        ...
        <?
        if ($arItem["CODE"] == "color_new"):
        ?>

            <div class="checkbox-tile checkbox-tile_size_big" style="display: inline-block;">
                <input
                        type="checkbox"
                        value="<?= $ar["HTML_VALUE"] ?>"
                        name="<?= $ar["CONTROL_NAME"] ?>"
                ...
                <label data-role="label_<?= $ar["CONTROL_ID"] ?>"
                       class="checkbox-tile__label checkbox-tile__label_color_green checkbox-tile__label_type_color"
                       style=" background-color: #<?= $ar["URL_ID"] ?> "
                       for="<?= $ar["CONTROL_ID"] ?> ">
                  <?= $ar["VALUE"];
                  if ($arParams["DISPLAY_ELEMENT_COUNT"] !== "N" && isset($ar["ELEMENT_COUNT"])): ?>&nbsp;(<span
                          data-role="count_<?= $ar["CONTROL_ID"] ?>"><?= $ar["ELEMENT_COUNT"]; ?></span>)<?
                  endif;
                  ?>
                </label>
        ...
        <?
        elseif ($arItem["CODE"] == "INPUT_SIZE_NUMERIC"):
          ?>
        ...
        <div class="bx_filter_button_box active">
            <div class="bx_filter_block">
                <div class="bx_filter_parameters_box_container">
                    <input class="bx_filter_search_button btn" type="submit" id="set_filter" name="set_filter"
                           value="Показать товары"/>

                    <div class="bx_filter_popup_result <?= $arParams["POPUP_POSITION"] ?>" id="modef" <?
                    if (!isset($arResult["ELEMENT_COUNT"])) {
                      echo 'style="display:none"';
                    }
                    ?> style="display: inline-block;">
                      <?= GetMessage("CT_BCSF_FILTER_COUNT", array(
...
<script>
    var smartFilter = new JCSmartFilter('<?= CUtil::JSEscape($arResult["FORM_ACTION"]) ?>', 'vertical');
</script>